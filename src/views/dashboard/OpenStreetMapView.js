...
export async function fetchGoogleSheetData(sheetCsvUrl, tabGid) {
  ...
  console.log('CSV Response:', csv.substring(0, 200) + '...'); // Show first 200 chars of response
  // Use PapaParse to robustly parse CSV, handling quoted fields
  const parsed = Papa.parse(csv, { header: true, skipEmptyLines: true });
  console.log('PapaParse result:', parsed);
  return parsed.data;
}

// Google Sheet base CSV URL and tab GIDs
const SHEET_BASE_URL = 'https://docs.google.com/spreadsheets/d/1c0-6pkvhvFntApsvgjCrLJrs0oU9e-L1wjfpsG80Ftk/export?format=csv';
const TABS = [
  { gid: '0', color: 'green', label: 'Current Active Customers' }, // Replace with actual gid for tab 1
  { gid: '364360587', color: 'red', label: 'New Customers' },    // Replace with actual gid for tab 2
];

const CUSTOMER_LOCATION_FIELD = 'موقع العميل';

// Create icons once outside component
const createIcon = (color) => new L.Icon({
  ...
  iconAnchor: [10, 32],
  popupAnchor: [1, -34],
  shadowSize: [32, 32], // Reduced shadow size
  shadowAnchor: [10, 32]
});
...
const OpenStreetMapView = () => {
  ...
  useEffect(() => {
    Promise.all(TABS.map(tab => fetchGoogleSheetData(SHEET_BASE_URL, tab.gid)))
      .then(results => {
  ...
  const markers = useMemo(() => 
    customersByTab.map((customers, tabIdx) =>
      customers.filter(c => {
        ...
        return {
          id: `${tabIdx}-${idx}`,
          position: [lat, lng],          isCurrentCustomer: tabIdx === 0,
          assignee: assignee
        };
      })
    ).flat(),
...
return (
    ...
    >
        ...
        >
            {markers.map(marker => {
                ...
                return (
                    ...
                    >
                        <Popup>
                            <div style={{ minWidth: 200 }}>                                <h6 style={{ marginBottom: 8 }}>
        ...
        {userLocation && (
            <Marker position={userLocation} icon={L.icon({ iconUrl: 'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-blue.png', iconSize: [20, 32], iconAnchor: [10, 32], popupAnchor: [1, -34], shadowUrl: markerShadow, shadowSize: [32, 32], shadowAnchor: [10, 32] })}>
